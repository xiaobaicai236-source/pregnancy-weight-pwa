...
- 修复孕周输入 26 时可能变成 46 的问题
- 输入过程中不再自动把临时的一位数字强制限制到 4–42
- 只有完成输入（change / blur）后才进行孕周范围校验
- 点击孕周或“天”输入框时自动选中原数字，便于直接替换
- 输入有效孕周时仍会即时刷新推荐体重和曲线
- Service Worker 与资源缓存升级至 v1.6.2


## v1.7.0 曲线显示优化

- 横坐标调整为 1、4、7、10……40 周
- 图表显示范围调整为完整孕期 1–40 周
- 保留实际体重记录和当前孕周标记


## v1.7.1 孕前体重提示优化
- 未设置孕前体重时显示“请输入准妈妈孕前体重”
- 设置后显示“孕前体重 xx.x kg”
