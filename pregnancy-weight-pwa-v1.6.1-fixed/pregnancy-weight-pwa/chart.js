(() => {
  ...
  function drawChart(c, opts={}){
    ...
    const minWeek=Number(opts.minWeek ?? 4);
    const maxWeek=Number(opts.maxWeek ?? 42);
    ...
    [4,12,20,28,36,42].filter(w=>w>=minWeek&&w<=maxWeek).forEach(w=>ctx.fillText(`${w}周`,x(w),height-pad.b+9));
    ctx.restore();
